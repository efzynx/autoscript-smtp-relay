...
app = Flask(__name__)
app.secret_key = 'smtp_relay_secret_key'

API_BASE_URL = os.environ.get('API_BASE_URL', 'http://localhost:5001')

@app.route('/')
def index():
    # The entire template is now self-contained and fixed
...
if __name__ == '__main__':
    ...
    # This template has the updated UI for adding senders via a modal
    template_content = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <script>
        const API_BASE_URL = window.location.origin.replace('5000', '5001') + '/api';
        let editSenderModal;
        let addSenderModal;
        ...
        async function loadSenders() {
            ...
            senders.forEach((sender, index) => {
                html += '<li class="list-group-item d-flex justify-content-between align-items-center">' +
                    ...
                    '<div>' +
                    '<button class="btn btn-sm btn-outline-primary me-2" onclick="openEditModal(' + index + ', \\\'' + sender.name + '\\\', \\\'' + sender.email + '\\\')"><i class="bi bi-pencil"></i></button>' +
        ...
        async function sendTestEmail(e) {
            ...
            const result = await apiFetch('/send_test_email', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(data)
            });
        ...
        async function flushQueue() {
            ...
            showMessage(data.message, data.status === 'success' ? 'info' : 'error');
            loadAllData();
        }
...
"""
    ...
    print(f"Starting Web UI on port 5000. It will connect to API server at {API_BASE_URL}")
